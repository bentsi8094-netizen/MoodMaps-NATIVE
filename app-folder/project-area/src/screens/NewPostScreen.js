...
export default function NewPostScreen() {
  ...
  const handleSubmit = async () => {
    ...
    if (result?.success) {
      ...
      setContent("");
      if (onPostSuccess) onPostSuccess(); // כאן הוא יחזיר אותך אוטומטית לפיד
    } else {
      Alert.alert("שגיאה", "הפרסום נכשל, נסה שוב.");
    }
};

  return (
    <ScrollView contentContainerStyle={styles.scrollContainer}>
      <Text style={styles.mainTitle}>מה המצב שלך כרגע?</Text>
      ...
      <GlassCard>
        ...
        >
          {isSubmitting ? <ActivityIndicator color="white" /> : <Text style={styles.submitText}>פרסם פוסט</Text>}
        </TouchableOpacity>
      </GlassCard>
    </ScrollView>
  );
...
const styles = StyleSheet.create({
  ...
  emojiSelector: { flexDirection: 'row', justifyContent: 'space-between', marginBottom: 25, width: '100%' },
  emojiBtn: { padding: 8, borderRadius: 12 },
  activeEmoji: { backgroundColor: 'rgba(255,255,255,0.2)', borderWidth: 1, borderColor: 'white' },
  input: { backgroundColor: 'rgba(0,0,0,0.2)', borderRadius: 15, padding: 15, color: 'white', minHeight: 80, textAlignVertical: 'top', marginBottom: 20 },
