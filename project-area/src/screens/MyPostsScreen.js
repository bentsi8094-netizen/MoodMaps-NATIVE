...
export default function MyPostsScreen() {
  ...
  const handleDelete = () => {
    Alert.alert("הסרת מוד", "להסיר את המוד שלך מהמפה?", [
      ...
      { text: "הסר", style: "destructive", onPress: () => removePost(currentUser.id) }
    ]);
  };
...
const styles = StyleSheet.create({
  ...
  postHeader: { alignItems: 'center', marginBottom: 20 },
  emoji: { fontSize: 60, marginBottom: 10 },
  content: { color: 'white', fontSize: 18, textAlign: 'center' },
  deleteBtn: { backgroundColor: '#ff4d4d', padding: 15, borderRadius: 25, alignItems: 'center' },
  deleteText: { color: 'white', fontWeight: 'bold' },
  emptyContainer: { flex: 1, justifyContent: 'center', alignItems: 'center' },
  emptyText: { color: 'white', fontSize: 18, opacity: 0.6 }
});